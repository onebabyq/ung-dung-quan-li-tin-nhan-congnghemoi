...
public interface RoomRepository extends CrudRepository<Room, Long>{
	...
	List<Room> findAll();
	@Query(value="SELECT r.* FROM room r JOIN account_room ar ON r.id = ar.room_id where r.type = 'Dual' AND ar.account_id = ?1 AND  r.id in (SELECT r.id FROM room r JOIN account_room ar ON r.id = ar.room_id where ar.account_id =?2)",nativeQuery = true)
